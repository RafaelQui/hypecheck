...
function Detail({ product: selectedProduct, onBack, onWant, wanted, wantError }: {product:Product;onBack:()=>void;onWant:()=>void;wanted:boolean;wantError?:boolean}) {
  ...
  const productQuery=useGetProduct(selectedProduct.id,{query:{queryKey:getGetProductQueryKey(selectedProduct.id),retry:false}});
  const reviewQuery=useGetProductReviews(selectedProduct.id,undefined,{query:{queryKey:getGetProductReviewsQueryKey(selectedProduct.id),retry:false,enabled:productQuery.isSuccess}});
...
export default function DiscoverTab() {
  const [onboarded,setOnboarded]=useState(false); const [tab,setTab]=useState('Discover'); const [optimisticAdds,setOptimisticAdds]=useState<string[]>([]); const [optimisticRemovals,setOptimisticRemovals]=useState<string[]>([]); const [detail,setDetail]=useState<Product|null>(null); const [wantError,setWantError]=useState(false);
  const { session }=useAuth();
  ...
  if(detail)return <Detail product={detail} onBack={()=>setDetail(null)} onWant={()=>wants.includes(detail.id)?removeWantById(detail.id):addWant(detail.id)} wanted={wants.includes(detail.id)} wantError={wantError}/>;
  return <>
