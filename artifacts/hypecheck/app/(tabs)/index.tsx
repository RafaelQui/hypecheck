...
import { ActivityIndicator, Alert, Animated, Dimensions, Image, Linking, Modal, PanResponder, Pressable, ScrollView, Share, StyleSheet, Text, TextInput, View } from 'react-native';
...
type Review = { id: string; user: string; avatarUrl?: string | null; rating: number; worthIt: boolean; helpful: boolean; text: string; date: string; likes: number; comments: number; imageUrl?: string | null; videoUrl?: string | null };
...
function VideoReviewPreview({ videoUrl, containerStyle }: { videoUrl:string; containerStyle?:any }) {
  ...
  return <View testID="video-review-player" accessibilityLabel="Video review player" style={[profileStyles.videoReviewCard,containerStyle]}>
    ...
    {status==='error'?<View testID="video-review-unavailable" accessibilityRole="alert" style={profileStyles.videoStatus}><Ionicons name="alert-circle-outline" size={28} color="#fff"/><Text style={profileStyles.videoStatusText}>Video unavailable</Text><Text style={profileStyles.videoErrorDetail}>This review video could not be loaded right now.</Text>{playbackError?<Text style={profileStyles.videoErrorDetail}>Please try again later.</Text>:null}</View>:null}
...
function Onboarding({ onDone }: {onDone:()=>void}) {
  ...
  const [email,setEmail]=useState(''); const [password,setPassword]=useState(''); const [login,setLogin]=useState(false); const [submitting,setSubmitting]=useState(false); const [authError,setAuthError]=useState<string|null>(null);
  const { signIn, signUp, isDemo } = useAuth();
  const next=async()=> {
    setAuthError(null);
    if(step<2){setStep(step+1);return;}
    if(!email.trim()||!password){setAuthError('Enter your email and a password with at least 6 characters.');return;}
  ...
  return <View style={s.onboarding}><View style={s.onboardMark}><Ionicons name="checkmark" size={40} color={colors.primaryForeground}/></View><Text style={s.onboardBrand}>HypeCheck</Text>{step===0 ? <><Text style={s.onboardTitle}>Discover products{'\n'}worth buying.</Text><Text style={s.onboardCopy}>Check the hype before you buy. Honest reviews from people who actually tried it.</Text></> : step===1 ? <><Text style={s.onboardTitleSmall}>What are you into?</Text><Text style={s.onboardCopy}>Pick a few interests so your feed feels like you.</Text><View style={s.categoryGrid}>{categories.map(c=><Chip key={c} label={c} active={selected.includes(c)} onPress={()=>setSelected(selected.includes(c)?selected.filter(x=>x!==c):[...selected,c])}/>)}</View></> : <><Text style={s.onboardTitleSmall}>{login?'Welcome back.':'Join the good finds.'}</Text><Text style={s.onboardCopy}>{login?'Sign in to access your Wants and reviews.':'Create an account to save Wants and share your honest take.'}</Text><TextInput testID="email-input" value={email} onChangeText={setEmail} placeholder="Email address" placeholderTextColor={colors.mutedForeground} style={s.input} keyboardType="email-address" autoCapitalize="none"/><PasswordInput key={login?'login-password':'signup-password'} value={password} onChangeText={setPassword} placeholder="Password (6+ characters)"/>{isDemo?<View testID="demo-mode-notice" style={{marginTop:10,padding:10,borderRadius:12,backgroundColor:'rgba(240,128,128,0.12)',borderWidth:1,borderColor:'rgba(240,128,128,0.3)'}}><Text style={{fontFamily:'Inter_600SemiBold',fontSize:12,color:colors.primary}}>Demo mode</Text><Text style={{fontFamily:'Inter_400Regular',fontSize:12,lineHeight:16,color:colors.foreground,marginTop:2}}>Backend isn&apos;t connected here — any email plus a 6+ character password will sign you in locally so you can try Wants, reviews, and your profile. Real accounts work when running on Replit.</Text></View>:null}{authError?<View testID="auth-error" style={{marginTop:10,padding:10,borderRadius:12,backgroundColor:'rgba(208,82,88,0.1)',borderWidth:1,borderColor:'rgba(208,82,88,0.4)'}}><Text style={{fontFamily:'Inter_600SemiBold',fontSize:12.5,lineHeight:17,color:'#B23840'}}>{authError}</Text></View>:null}<Pressable onPress={()=>{setAuthError(null);setLogin(value=>!value);}}><Text style={s.skip}>{login?'Need an account? Create one':'Already have an account? Sign in'}</Text></Pressable></>}<View style={s.onboardFooter}><View style={s.dots}>{[0,1,2].map(i=><View key={i} style={[s.dot,i===step&&s.dotActive]}/>)}</View><Pressable testID={step===2?(login?'sign-in-submit':'sign-up-submit'):'onboarding-continue'} disabled={submitting} style={s.primaryButton} onPress={next}><Text style={s.primaryText}>{step===2?(submitting?'Working…':login?'Sign in':'Create account'):'Continue'}</Text><Feather name="arrow-right" size={18} color="#fff"/></Pressable><Pressable testID="onboarding-skip" onPress={onDone}><Text style={s.skip}>Skip for now</Text></Pressable></View></View>;
}
...
function Discover({ catalog, wants, setWants, onDetail, onProfile }: {catalog:Product[];wants:string[];setWants:(v:string[])=>void;onDetail:(p:Product)=>void;onProfile:()=>void}) {
  ...
  return <View style={[s.screen,{paddingBottom:76}]}>
    ...
    <View style={s.cardStage}>
      <View style={[s.productCard,s.stackedCard]}><Image source={next.image} style={s.cardImage}/><View style={s.stackedTint} pointerEvents="none"/></View>
      <Animated.View {...pan.panHandlers} style={[s.productCard,s.activeCard,{transform:[{translateX:position.x},{translateY:position.y},{rotate}]}]}>
        ...
        <Pressable testID={`product-card-${current.id}`} onPress={()=>{if(!didDrag.current)onDetail(current);}} style={s.cardFill}>
          <Image source={current.image} style={s.cardImageFull}/>
          <LinearGradient colors={['rgba(0,0,0,0.05)','rgba(0,0,0,0)','rgba(0,0,0,0.45)','rgba(0,0,0,0.88)']} locations={[0,0.35,0.7,1]} style={s.cardGradient} pointerEvents="none"/>
          <View style={discoverStyles.hypeChip}><Text style={discoverStyles.hypeChipEmoji}>🔥</Text><View><Text style={discoverStyles.hypeChipNumber}>{current.recommend ?? '—'}%</Text><Text style={discoverStyles.hypeChipLabel}>Worth the Hype</Text></View></View>
          <View style={discoverStyles.cardBottom}>
            <View style={discoverStyles.categoryPill}><Text style={discoverStyles.categoryPillText}>{current.category.toUpperCase()}</Text></View>
            <Text style={s.cardTitle} numberOfLines={2}>{current.name}</Text>
            <View style={discoverStyles.statsRow}>
              <Text style={s.cardPrice}>${current.price.toFixed(2)}</Text>
              <View style={discoverStyles.statDot}/>
              <View style={discoverStyles.ratingRow}><Ionicons name="star" size={14} color="#FFD667"/><Text style={discoverStyles.ratingValue}>{current.rating.toFixed(1)}</Text><Text style={discoverStyles.reviewCountLight}>({current.reviews.toLocaleString()})</Text></View>
            </View>
            <View style={discoverStyles.quoteCard}>
              <View style={discoverStyles.quoteAvatar}><Text style={s.avatarText}>{current.reviewer[0].toUpperCase()}</Text></View>
              <View style={{flex:1}}>
                <View style={discoverStyles.quoteHead}>
                  <Text style={discoverStyles.quoteAuthor}>@{current.reviewer}</Text>
                  <Pressable onPress={(e)=>{e.stopPropagation?.();setFollowing(value=>!value);}} style={[discoverStyles.followButton,following&&discoverStyles.followButtonActive]}><Text style={[discoverStyles.followText,following&&discoverStyles.followTextActive]}>{following?'Following':'Follow'}</Text></Pressable>
                </View>
                <Text style={discoverStyles.quoteBody} numberOfLines={2}>&ldquo;{current.caption}&rdquo;</Text>
              </View>
            </View>
          </View>
        </Pressable>
    ...
    <View style={s.swipeActions}>
      <Pressable testID="pass-product" accessibilityRole="button" accessibilityLabel="Pass on this product" disabled={busy.current} onPress={()=>finish(-1)} style={({pressed})=>[s.roundAction,s.passAction,pressed&&discoverStyles.actionPressed]}><Ionicons name="close" size={30} color="#D05258"/></Pressable>
      <Pressable testID="detail-open" accessibilityRole="button" accessibilityLabel="See product details" onPress={()=>onDetail(current)} style={({pressed})=>[discoverStyles.infoAction,pressed&&discoverStyles.actionPressed]}><Feather name="info" size={22} color={colors.foreground}/></Pressable>
      <Pressable testID="want-product" accessibilityRole="button" accessibilityLabel="Add to Wants" disabled={busy.current} onPress={()=>finish(1)} style={({pressed})=>[s.roundAction,s.wantAction,pressed&&discoverStyles.actionPressed]}><Ionicons name="heart" size={30} color="#fff"/></Pressable>
    </View>
...
function Detail({ product: selectedProduct, onBack, onWant, wanted, wantError, usingPlaceholders }: {product:Product;onBack:()=>void;onWant:()=>void;wanted:boolean;wantError?:boolean;usingPlaceholders?:boolean}) {
  const [tab,setTab]=useState<'Reviews'|'Photos'|'Videos'>('Reviews');
  const [actionError,setActionError]=useState<string|null>(null);
  const [pendingHelpful,setPendingHelpful]=useState<string|null>(null);
  const {session}=useAuth();
  const queryClient=useQueryClient();
  const productQuery=useGetProduct(selectedProduct.id,{query:{queryKey:getGetProductQueryKey(selectedProduct.id),retry:false,enabled:!usingPlaceholders}});
  const reviewQuery=useGetProductReviews(selectedProduct.id,undefined,{query:{queryKey:getGetProductReviewsQueryKey(selectedProduct.id),retry:false,enabled:!usingPlaceholders&&productQuery.isSuccess}});
  const liveReviews:Review[]=reviewQuery.data?.items.map(review=>({id:review.id,user:review.authorUsername||'hypecheck-user',avatarUrl:review.authorAvatarUrl,rating:review.rating,worthIt:review.worthIt,helpful:review.viewerHasLiked??false,text:review.reviewText,date:new Date(review.createdAt).toLocaleDateString(),likes:review.likeCount??0,comments:review.commentCount??0,imageUrl:review.photoUrl,videoUrl:review.videoUrl}))??[];
  const shownReviews=tab==='Photos'?liveReviews.filter(review=>Boolean(review.imageUrl)):tab==='Videos'?liveReviews.filter(review=>Boolean(review.videoUrl)):liveReviews;
  const primaryReviewVideo=liveReviews.find(review=>Boolean(review.videoUrl))?.videoUrl??null;
  // In placeholder mode we synthesize a product record from the swipe card data.
  const placeholderProduct=usingPlaceholders?{id:selectedProduct.id,name:selectedProduct.name,description:selectedProduct.description,price:selectedProduct.price,category:selectedProduct.category,retailer:selectedProduct.retailer??null,imageUrl:selectedProduct.imageUrl??null,storeUrl:selectedProduct.storeUrl??null,rating:selectedProduct.rating,reviewCount:selectedProduct.reviews,worthTheHype:selectedProduct.recommend}:null;
  const product=usingPlaceholders?placeholderProduct:productQuery.data;
  const placeholderReviews:Review[]=usingPlaceholders?[{id:`${selectedProduct.id}-r1`,user:selectedProduct.reviewer,avatarUrl:null,rating:selectedProduct.rating,worthIt:(selectedProduct.recommend??0)>=70,helpful:false,text:selectedProduct.caption,date:'Recently',likes:Math.floor(selectedProduct.likes/50),comments:selectedProduct.comments,imageUrl:null,videoUrl:null},{id:`${selectedProduct.id}-r2`,user:'hypechecker',avatarUrl:null,rating:Math.max(1,Math.round(selectedProduct.rating)-1),worthIt:(selectedProduct.recommend??0)>=80,helpful:false,text:'Solid pick — arrived faster than expected and feels sturdier than the price suggests.',date:'2d ago',likes:42,comments:5,imageUrl:null,videoUrl:null}]:[];
  const displayReviews=usingPlaceholders?(tab==='Photos'?[]:tab==='Videos'?[]:placeholderReviews):shownReviews;
  const handleHelpful=async(review:Review, nextHelpful:boolean)=>{
    if(!session){Alert.alert('Sign in to mark helpful','Create an account or sign in to mark reviews as helpful.');return;}
    const reviewQueryKey=getGetProductReviewsQueryKey(selectedProduct.id);
    const applyHelpful=(helpful:boolean,likeCount:number)=>queryClient.setQueryData(reviewQueryKey,(current:any)=>current?{...current,items:current.items.map((item:any)=>item.id===review.id?{...item,viewerHasLiked:helpful,likeCount}:item)}:current);
    const previous={helpful:review.helpful,likeCount:review.likes};
    const optimisticCount=Math.max(0,previous.likeCount+(nextHelpful?1:-1));
    setActionError(null);setPendingHelpful(review.id);applyHelpful(nextHelpful,optimisticCount);
    try{const result=await setReviewHelpful(review.id,nextHelpful);applyHelpful(result.helpful,result.likeCount);queryClient.invalidateQueries({queryKey:reviewQueryKey});}
    catch{applyHelpful(previous.helpful,previous.likeCount);setActionError('We could not update that Helpful vote. Please try again.');}
    finally{setPendingHelpful(null);}
  };
  const handleShare=async()=>{
    if(!product)return;
    try{await Share.share({title:product.name,message:`Check out ${product.name} on HypeCheck.${product.storeUrl?` ${product.storeUrl}`:''}`});}
    catch{setActionError('We could not open sharing right now. Please try again.');}
  };
  const handleStore=async()=>{
    const storeUrl=product?.storeUrl?.trim();
    if(!storeUrl)return;
    try{const supported=await Linking.canOpenURL(storeUrl);if(!supported)throw new Error('unsupported');await Linking.openURL(storeUrl);}
    catch{setActionError('We could not open this retailer right now. Please try again.');}
  };
  if(!usingPlaceholders&&productQuery.isLoading)return <View style={[s.page,s.detailState]}><ActivityIndicator color={colors.primary}/><Text style={s.emptyTitle}>Loading product…</Text><Text style={s.muted}>Getting the latest details and reviews.</Text><Pressable onPress={onBack} style={s.outlineButton}><Text style={s.outlineText}>Go back</Text></Pressable></View>;
  if(!usingPlaceholders&&(productQuery.isError||!product))return <View style={[s.page,s.detailState]}><Ionicons name="alert-circle-outline" size={34} color={colors.primary}/><Text style={s.emptyTitle}>This product isn&apos;t available</Text><Text style={[s.muted,s.emptyCopy]}>It may have been removed or is temporarily unavailable.</Text><View style={s.detailStateActions}><Pressable onPress={onBack} style={s.outlineButton}><Text style={s.outlineText}>Go back</Text></Pressable><Pressable onPress={()=>productQuery.refetch()} style={s.primaryButton}><Text style={s.primaryText}>Try again</Text></Pressable></View></View>;
  if(!product)return null;
  const noRating=product.rating==null||product.reviewCount===0;
  return <ScrollView style={s.page} contentContainerStyle={{paddingBottom:40}}>
    <View style={s.detailHero}>
      {product.imageUrl?<Image source={{uri:product.imageUrl}} style={s.detailImage}/>:usingPlaceholders?<Image source={selectedProduct.image} style={s.detailImage}/>:primaryReviewVideo?<VideoReviewPreview videoUrl={primaryReviewVideo} containerStyle={s.detailHeroVideo}/>:<View style={s.detailMediaFallback}><Ionicons name="images-outline" size={34} color={colors.primary}/><Text style={s.detailMediaFallbackText}>No product media yet</Text></View>}
      <Pressable accessibilityRole="button" accessibilityLabel="Back to products" onPress={onBack} style={s.backButton}><Feather name="arrow-left" size={21} color="#fff"/></Pressable><Pressable accessibilityRole="button" accessibilityLabel="Share product" onPress={handleShare} style={s.shareButton}><Feather name="share-2" size={19} color="#fff"/></Pressable>
    </View>
    <View style={s.detailBody}>
      <Text style={s.categoryLabel}>{product.category.toUpperCase()}</Text><Text style={s.detailTitle}>{product.name}</Text><Text style={s.detailRetailer}>{product.retailer||'Retailer not listed'}</Text>{product.description?<Text style={s.detailDescription}>{product.description}</Text>:null}
      <View style={s.detailStats}><View><Text style={s.detailPrice}>${product.price.toFixed(2)}</Text>{noRating?<Text style={s.unrated}>Not rated yet</Text>:<Rating value={product.rating??0}/>}</View><View style={s.statDivider}/><View><Text style={s.statBig}>{product.worthTheHype==null?'—':`${product.worthTheHype}%`}</Text><Text style={s.statLabel}>Worth the hype</Text><Text style={s.statLabel}>{product.reviewCount} {product.reviewCount===1?'review':'reviews'}</Text></View></View>
      {wantError?<View style={s.detailNotice}><Ionicons name="alert-circle-outline" size={17} color={colors.primary}/><Text style={s.detailNoticeText}>We couldn&apos;t update your Wants. Please try again.</Text></View>:null}
      {actionError?<View style={s.detailNotice}><Ionicons name="alert-circle-outline" size={17} color={colors.primary}/><Text style={s.detailNoticeText}>{actionError}</Text></View>:null}
      <View style={s.detailButtons}><Pressable accessibilityRole="button" accessibilityLabel={wanted?'Remove from Wants':'Save to Wants'} onPress={onWant} style={[s.primaryButton,s.flexButton]}><Ionicons name={wanted?'heart':'heart-outline'} size={19} color="#fff"/><Text style={s.primaryText}>{wanted?'Saved':'Want'}</Text></Pressable>{product.storeUrl?.trim()?<Pressable accessibilityRole="button" accessibilityLabel={`View on ${product.retailer||'retailer'}`} onPress={handleStore} style={s.outlineButton}><Text style={s.outlineText}>View on {product.retailer||'retailer'}</Text><Feather name="external-link" size={16} color={colors.primary}/></Pressable>:null}</View>
      <View style={s.sectionHeader}><Text style={s.sectionTitle}>Community reviews</Text><Text style={s.reviewCount}>{product.reviewCount?`${product.reviewCount.toLocaleString()} total`:'Not rated yet'}</Text></View>
      <View style={s.tabs}>{(['Reviews','Videos','Photos'] as const).map(tabName=><Pressable testID={`product-detail-${tabName.toLowerCase()}-tab`} accessibilityRole="tab" accessibilityState={{selected:tab===tabName}} key={tabName} onPress={()=>setTab(tabName)} style={[s.detailTab,tab===tabName&&s.detailTabActive]}><Text style={[s.detailTabText,tab===tabName&&s.detailTabTextActive]}>{tabName}</Text></Pressable>)}</View>
      {reviewQuery.isLoading&&!usingPlaceholders?<View style={s.reviewState}><ActivityIndicator color={colors.primary}/><Text style={s.muted}>Loading reviews…</Text></View>:reviewQuery.isError&&!usingPlaceholders?<View style={s.reviewState}><Text style={s.emptyTitle}>Reviews are unavailable</Text><Text style={s.muted}>Please try again in a moment.</Text><Pressable onPress={()=>reviewQuery.refetch()} style={s.outlineButton}><Text style={s.outlineText}>Try again</Text></Pressable></View>:displayReviews.length?displayReviews.map(review=>{const isHelpful=review.helpful;const likeCount=review.likes;return <View key={review.id} style={s.reviewItem}>{review.imageUrl?<Image source={{uri:review.imageUrl}} style={s.reviewThumb}/>:null}{review.videoUrl?<VideoReviewPreview videoUrl={review.videoUrl}/>:null}<View style={s.reviewHead}>{review.avatarUrl?<Image source={{uri:review.avatarUrl}} style={s.reviewAvatar}/>:<View style={s.miniAvatar}><Text style={s.avatarText}>{review.user[0].toUpperCase()}</Text></View>}<View style={{flex:1}}><Text style={s.reviewerName}>@{review.user}</Text><Rating value={review.rating} size={12}/></View><Text style={s.date}>{review.date}</Text></View><View style={[s.worthItBadge,review.worthIt?s.worthItYes:s.worthItNo]}><Ionicons name={review.worthIt?'checkmark-circle':'close-circle'} size={14} color={review.worthIt?'#138A62':'#B94242'}/><Text style={[s.worthItText,review.worthIt?s.worthItYesText:s.worthItNoText]}>Worth It: {review.worthIt?'Yes':'No'}</Text></View>{review.text?<Text style={s.reviewText}>{review.text}</Text>:null}<View style={s.reviewFooter}><Pressable disabled={pendingHelpful===review.id} onPress={()=>handleHelpful(review,!isHelpful)} style={[s.helpfulButton,isHelpful&&s.helpfulButtonActive]}><Feather name="thumbs-up" size={14} color={isHelpful?colors.primary:colors.mutedForeground}/><Text style={[s.helpful,isHelpful&&s.helpfulActive]}>{pendingHelpful===review.id?'Saving…':`Helpful ${likeCount}`}</Text></Pressable><Text style={s.helpful}><Feather name="message-circle" size={14} color={colors.mutedForeground}/> {review.comments}</Text></View></View>}):<View style={s.emptyTab}><Ionicons name={tab==='Photos'?'images-outline':tab==='Videos'?'videocam-outline':'chatbubble-ellipses-outline'} size={28} color={colors.primary}/><Text style={s.emptyTitle}>No {tab.toLowerCase()} yet</Text><Text style={s.muted}>Be the first to add one.</Text></View>}
    </View>
  </ScrollView>;
}
...
export default function DiscoverTab() {
  const [onboarded,setOnboarded]=useState(false); const [tab,setTab]=useState('Discover'); const [optimisticAdds,setOptimisticAdds]=useState<string[]>([]); const [optimisticRemovals,setOptimisticRemovals]=useState<string[]>([]); const [detail,setDetail]=useState<Product|null>(null); const [wantError,setWantError]=useState(false); const [localWants,setLocalWants]=useState<string[]>([]);
  const { session, loading: authLoading }=useAuth();
  // A persisted session (from a previous demo or real sign-in) should skip
  // onboarding — otherwise a page refresh sends signed-in users back to step 0.
  const skipOnboarding = onboarded || Boolean(session);
  const productsQuery=useListProducts(undefined,{query:{queryKey:getListProductsQueryKey(),retry:false}});
  const liveCatalog=productsQuery.data?.items?.map(toDisplayProduct)??[];
  // Fallback: if Supabase is unreachable, empty, or still loading on web preview,
  // use the built-in placeholder products so the Discover experience always works.
  const usingPlaceholders=liveCatalog.length===0;
  const catalog=usingPlaceholders?products:liveCatalog;
  ...
  const remoteWants=wantsQuery.data?.items.map(item=>item.productId)??[]; const wants=Array.from(new Set([...remoteWants,...localWants,...optimisticAdds])).filter(id=>!optimisticRemovals.includes(id));
  const addWant=(id:string)=>{if(wants.includes(id))return;setWantError(false);setOptimisticRemovals(value=>value.filter(item=>item!==id));if(!session||usingPlaceholders){setLocalWants(value=>value.includes(id)?value:[...value,id]);return;}setOptimisticAdds(value=>[...value,id]);saveWant.mutate({data:{productId:id}},{onSuccess:()=>{setWantError(false);wantsQuery.refetch();},onError:()=>{setOptimisticAdds(value=>value.filter(item=>item!==id));setWantError(true);Alert.alert('Could not save Want','Your Wants could not be updated. Please try again.');}});};
  const removeWantById=(id:string)=>{setWantError(false);setLocalWants(value=>value.filter(item=>item!==id));if(!session||usingPlaceholders){setOptimisticAdds(value=>value.filter(item=>item!==id));return;}setOptimisticRemovals(value=>[...value,id]);setOptimisticAdds(value=>value.filter(item=>item!==id));removeWant.mutate({params:{productId:id}},{onSuccess:()=>{setWantError(false);wantsQuery.refetch();},onError:()=>{setOptimisticRemovals(value=>value.filter(item=>item!==id));setWantError(true);Alert.alert('Could not remove Want','Your Wants could not be updated. Please try again.');}});};
  ...
  if(!skipOnboarding)return <Onboarding onDone={()=>setOnboarded(true)}/>;
  if(detail)return <Detail product={detail} onBack={()=>setDetail(null)} onWant={()=>wants.includes(detail.id)?removeWantById(detail.id):addWant(detail.id)} wanted={wants.includes(detail.id)} wantError={wantError} usingPlaceholders={usingPlaceholders}/>;
  return <>
...
const discoverStyles = StyleSheet.create({
  ...
  // Card overlay layout
  cardBottom:{position:'absolute',left:18,right:18,bottom:18,gap:10},
  categoryPill:{alignSelf:'flex-start',backgroundColor:'rgba(255,255,255,0.18)',borderWidth:1,borderColor:'rgba(255,255,255,0.35)',borderRadius:999,paddingHorizontal:10,paddingVertical:4},
  categoryPillText:{fontFamily:'Inter_700Bold',fontSize:10,color:'#fff',letterSpacing:1.2},
  statsRow:{flexDirection:'row',alignItems:'center',gap:10,marginTop:2},
  statDot:{width:4,height:4,borderRadius:2,backgroundColor:'rgba(255,255,255,0.55)'},
  ratingRow:{flexDirection:'row',alignItems:'center',gap:4},
  ratingValue:{fontFamily:'Inter_700Bold',fontSize:14,color:'#fff'},
  reviewCountLight:{fontFamily:'Inter_500Medium',fontSize:12,color:'rgba(255,255,255,0.75)'},
  // Worth the Hype floating chip (top-right)
  hypeChip:{position:'absolute',top:16,right:16,flexDirection:'row',alignItems:'center',gap:8,backgroundColor:'rgba(23,21,26,0.82)',borderWidth:1,borderColor:'rgba(255,255,255,0.15)',borderRadius:14,paddingHorizontal:12,paddingVertical:8,shadowColor:'#000',shadowOpacity:.25,shadowRadius:8,shadowOffset:{width:0,height:4}},
  hypeChipEmoji:{fontSize:20},
  hypeChipNumber:{fontFamily:'Inter_700Bold',fontSize:16,color:'#fff',lineHeight:18},
  hypeChipLabel:{fontFamily:'Inter_600SemiBold',fontSize:9,color:'rgba(255,255,255,0.8)',letterSpacing:.5,marginTop:1},
  // Reviewer quote card inside product card
  quoteCard:{flexDirection:'row',gap:10,backgroundColor:'rgba(255,255,255,0.14)',borderWidth:1,borderColor:'rgba(255,255,255,0.22)',borderRadius:16,padding:10,marginTop:4},
  quoteAvatar:{width:32,height:32,borderRadius:16,backgroundColor:'#F4C8AE',alignItems:'center',justifyContent:'center'},
  quoteHead:{flexDirection:'row',alignItems:'center',gap:8},
  quoteAuthor:{fontFamily:'Inter_700Bold',fontSize:12,color:'#fff'},
  quoteBody:{fontFamily:'Inter_500Medium',fontSize:12.5,lineHeight:17,color:'rgba(255,255,255,0.94)',marginTop:2},
  followButton:{marginLeft:'auto',borderWidth:1,borderColor:'rgba(255,255,255,0.5)',borderRadius:10,paddingHorizontal:8,paddingVertical:3},
  followButtonActive:{backgroundColor:'#fff',borderColor:'#fff'},
  followText:{fontFamily:'Inter_700Bold',fontSize:10,color:'#fff'},
  followTextActive:{color:colors.primary},
  // Center action button (info)
  infoAction:{width:52,height:52,borderRadius:26,alignItems:'center',justifyContent:'center',backgroundColor:'#fff',borderWidth:1,borderColor:colors.border,shadowColor:'#000',shadowOpacity:.05,shadowRadius:6,elevation:2},
  actionPressed:{transform:[{scale:0.94}],opacity:0.85},
