...
import { Alert, Animated, Dimensions, Image, Modal, PanResponder, Pressable, ScrollView, StyleSheet, Text, TextInput, View } from 'react-native';
...
type Review = { user: string; rating: number; text: string; date: string; likes: number; comments: number; image?: any };
...
function Detail({ product: fallbackProduct, onBack, onWant, wanted }: {product:Product;onBack:()=>void;onWant:()=>void;wanted:boolean}) { const [tab,setTab]=useState('Reviews'); const productQuery=useGetProduct(fallbackProduct.id,{query:{queryKey:getGetProductQueryKey(fallbackProduct.id),retry:false}}); const remote=productQuery.data; const product:Product=remote?{...fallbackProduct,id:remote.id,name:remote.name,category:remote.category||fallbackProduct.category,price:remote.price,description:remote.description||'',retailer:remote.retailer,imageUrl:remote.imageUrl,image:remote.imageUrl?{uri:remote.imageUrl}:fallbackProduct.image,storeUrl:remote.storeUrl,rating:remote.rating??0,reviews:remote.reviewCount,recommend:remote.worthTheHype??null}:fallbackProduct; const reviewQuery=useGetProductReviews(product.id,undefined,{query:{queryKey:getGetProductReviewsQueryKey(product.id),retry:false}}); const liveReviews=reviewQuery.data?.items.map(r=>({user:r.authorUsername||'hypecheck-user',rating:r.rating,text:r.reviewText,date:new Date(r.createdAt).toLocaleDateString(),likes:0,comments:0,image:r.photoUrl?{uri:r.photoUrl}:undefined}))??reviews; return <ScrollView style={s.page} contentContainerStyle={{paddingBottom:40}}><View style={s.detailHero}><Image source={product.image} style={s.detailImage}/><Pressable onPress={onBack} style={s.backButton}><Feather name="arrow-left" size={21} color="#fff"/></Pressable><Pressable style={s.shareButton}><Feather name="share-2" size={19} color="#fff"/></Pressable></View><View style={s.detailBody}><Text style={s.categoryLabel}>{product.category.toUpperCase()}</Text><Text style={s.detailTitle}>{product.name}</Text><Text style={s.detailDescription}>{product.description}</Text><View style={s.detailStats}><View><Text style={s.detailPrice}>${product.price.toFixed(2)}</Text><Rating value={product.rating}/></View><View style={s.statDivider}/><View><Text style={s.statBig}>{product.recommend===null?'—':`${product.recommend}%`}</Text><Text style={s.statLabel}>Worth the hype</Text></View></View><View style={s.detailButtons}><Pressable onPress={onWant} style={[s.primaryButton,s.flexButton]}><Ionicons name={wanted?'heart':'heart-outline'} size={19} color="#fff"/><Text style={s.primaryText}>{wanted?'In your Wants':'Want it'}</Text></Pressable><Pressable onPress={()=>product.storeUrl?Alert.alert('Store link',product.storeUrl):Alert.alert('No store link','This product does not have a store link yet.')} style={s.outlineButton}><Text style={s.outlineText}>View Product</Text><Feather name="external-link" size={16} color={colors.primary}/></Pressable></View><View style={s.sectionHeader}><Text style={s.sectionTitle}>Community reviews</Text><Text style={s.reviewCount}>{product.reviews?`${product.reviews.toLocaleString()} total`:'Not rated yet'}</Text></View><View style={s.tabs}>{['Reviews','Photos','Videos'].map(t=><Pressable key={t} onPress={()=>setTab(t)} style={[s.detailTab,tab===t&&s.detailTabActive]}><Text style={[s.detailTabText,tab===t&&s.detailTabTextActive]}>{t}</Text></Pressable>)}</View>{tab==='Reviews'?liveReviews.map((r,i)=><View key={i} style={s.reviewItem}>{r.image&&<Image source={r.image} style={s.reviewThumb}/>}<View style={s.reviewHead}><View style={s.miniAvatar}><Text style={s.avatarText}>{r.user[0].toUpperCase()}</Text></View><View style={{flex:1}}><Text style={s.reviewerName}>@{r.user}</Text><Rating value={r.rating} size={12}/></View><Text style={s.date}>{r.date}</Text></View><Text style={s.reviewText}>{r.text}</Text><View style={s.reviewFooter}><Text style={s.helpful}><Feather name="thumbs-up" size={14} color={colors.mutedForeground}/> Helpful {r.likes}</Text><Text style={s.helpful}><Feather name="message-circle" size={14} color={colors.mutedForeground}/> {r.comments}</Text></View></View>):<View style={s.emptyTab}><Ionicons name={tab==='Photos'?'images-outline':'videocam-outline'} size={28} color={colors.primary}/><Text style={s.emptyTitle}>No {tab.toLowerCase()} yet</Text><Text style={s.muted}>Be the first to add one.</Text></View>}</View></ScrollView>; }
