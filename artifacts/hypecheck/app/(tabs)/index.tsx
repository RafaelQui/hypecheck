...
function VideoReviewPreview({ videoUrl, containerStyle }: { videoUrl:string; containerStyle?:any }) {
  ...
  return <View style={[profileStyles.videoReviewCard,containerStyle]}>
...
function Detail({ product: selectedProduct, onBack, onWant, wanted, wantError }: {product:Product;onBack:()=>void;onWant:()=>void;wanted:boolean;wantError?:boolean}) {
  ...
  return <ScrollView style={s.page} contentContainerStyle={{paddingBottom:40}}>
    ...
    <View style={s.detailBody}>
      ...
      <View style={s.tabs}>{(['Reviews','Videos','Photos'] as const).map(tabName=><Pressable accessibilityRole="tab" accessibilityState={{selected:tab===tabName}} key={tabName} onPress={()=>setTab(tabName)} style={[s.detailTab,tab===tabName&&s.detailTabActive]}><Text style={[s.detailTabText,tab===tabName&&s.detailTabTextActive]}>{tabName}</Text></Pressable>)}</View>
